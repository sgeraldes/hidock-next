...
class HiDockFirmwareDownloader:
    ...
    def get_latest_firmware(self, device_model="hidock-h1e", current_version="328196"):
        ...
        data = {"version": current_version, "model": device_model}
            ...
            try:
                ...
                if error_code == "0" and root.find("data") is not None:
                    ...
                    firmware_data = {
                        ...
                        "versionCode": (
                            data_elem.find("versionCode").text if data_elem.find("versionCode") is not None else None
                        ),
                        "versionNumber": (
                            data_elem.find("versionNumber").text
                            if data_elem.find("versionNumber") is not None
                            else None
                        ),
                        "signature": (
                            data_elem.find("signature").text if data_elem.find("signature") is not None else None
                        ),
                        "fileName": data_elem.find("fileName").text if data_elem.find("fileName") is not None else None,
                        "fileLength": (
                            data_elem.find("fileLength").text if data_elem.find("fileLength") is not None else None
                        ),
    ...
    def download_firmware(self, firmware_info, output_path=None):
        ...
        try:
            ...
            if binary_data.startswith(b"ACTTEST0"):
                print("  Format: Valid ACTTEST0 firmware [+]")
            else:
                print("  Format: Unknown format (not ACTTEST0)")
    ...
    def extract_firmware_from_har(self, har_file_path, firmware_id, output_path=None):
        ...
        try:
            ...
            for entry in har_data["log"]["entries"]:
                if (
                    entry["response"]["content"].get("size") == 3451904
                    and entry["response"]["content"].get("mimeType") == "application/octet-stream"
                ):
            ...
            # Create output directory if not specified
            if not output_path:
                output_dir = "firmware/extracted_from_har"
                os.makedirs(output_dir, exist_ok=True)
...
def main():
    ...
    for device in devices:
        ...
        firmware_info, download_path = downloader.get_and_download_latest(device["model"], device["version"])
