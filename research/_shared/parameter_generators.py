...
class ParameterGenerator:
    ...
    def generate_authentication_patterns(self) -> List[Tuple[bytes, str]]:
        ...
        auth_numbers = [0x12345678, 0x87654321, 0xAAAAAAAA, 0x55555555, 0x00000001, 0xFFFFFFFF, 0x12341234, 0xABCDABCD]
    ...
    def generate_memory_address_patterns(self) -> List[Tuple[bytes, str]]:
        ...
        for addr in addresses:
            for length in lengths:
                params.append((struct.pack("<II", addr, length), f"Memory read: addr=0x{addr:08X}, len={length}"))
