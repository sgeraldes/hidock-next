...
class HiDockJensen:
        ...
        try:
            ...
            if "no langid" in str(e_val).lower():
                logger.warning(
                    "Jensen",
                    "_find_device",
                    f"HiDock device found but appears to be in use by another application. "
                    f"Please close any other apps using the device and try again.",
                )
        ...
        try:
            ...
            try:
                ...
                if e_cfg.errno == 16:  # Resource busy
                    logger.warning(  # Changed from error to warning - this is expected
                        "Jensen",
                        "_attempt_connection",
                        "Cannot connect: Device is currently in use by another application. "
                        "Please close any other apps using the HiDock device and try again.",
                    )
                    self._increment_error_count("connection_lost")
                    ...
                    logger.warning(  # Changed from error to warning - this is expected
                        "Jensen",
                        "_attempt_connection",
                        "Cannot connect: Access denied. Device may be in use by another application "
                        "or insufficient permissions. Please close other apps or try running as administrator.",
                    )
                    self._increment_error_count("connection_lost")
            ...
            try:
                ...
                if e_claim.errno == 16:  # Resource busy (LIBUSB_ERROR_BUSY)
                    logger.warning(  # Changed from error to warning - this is expected
                        "Jensen",
                        "_attempt_connection",
                        f"Cannot connect: Device is currently in use by another application. "
                        f"Please close any other apps using the HiDock device and try again.",
                    )
                    self._increment_error_count("connection_lost")
    ...
    def list_files_with_retry(self, timeout_s=20, max_retries=2):
        ...
        for attempt in range(max_retries + 1):
            logger.info("Jensen", "list_files_with_retry", 
                       f"Attempt {attempt + 1}/{max_retries + 1} to get file list")
            ...
            if result and result.get("incomplete"):
                ...
                logger.error("Jensen", "list_files_with_retry", 
                           f"Attempt {attempt + 1} failed completely")
                if attempt < max_retries:
