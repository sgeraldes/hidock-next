...
class DiscoveryResultsAnalyzer:
    ...
    def analyze_parameter_patterns(self):
        ...
        for length in sorted(length_groups.keys()):
            ...
            print(
                f"  {length} bytes: {len(results)} tests, {success_count} successful ({(success_count/len(results))*100:.1f}%)"
            )
    ...
    def generate_recommendations(self):
        ...
        for length in sorted(length_total.keys()):
            ...
            print(
                f"  {length} bytes: {success_rate:.1f}% success rate ({length_success[length]}/{length_total[length]})"
            )
...
def main():
    ...
    parser.add_argument("results_file", nargs="?", help="JSON results file to analyze")
    parser.add_argument("--export", "-e", action="store_true", help="Export detailed report to file")
