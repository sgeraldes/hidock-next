            ...
            try:
                ...
                if cmd_id == 1:
                    ...
                    if offset + 6 <= len(self.binary_data):
                        ...
                        if (
                            0x08000000 <= potential_handler <= 0x08100000  # Flash
                            or 0x20000000 <= potential_handler <= 0x20080000  # SRAM
                            or 0x00000000 <= potential_handler <= 0x00100000
                        ):  # Vector table
                            ...
                            for next_cmd in range(2, 21):
                                if check_offset + 6 <= len(self.binary_data):
                                    ...
                                    next_handler = struct.unpack(
                                        "<I", self.binary_data[check_offset + 2 : check_offset + 6]
                                    )[0]

                                    if next_id == next_cmd and (
                                        0x08000000 <= next_handler <= 0x08100000
                                        or 0x20000000 <= next_handler <= 0x20080000
                                        or 0x00000000 <= next_handler <= 0x00100000
                                    ):
                                        valid_entries += 1
                                        check_offset += 6
                                    else:
                ...
                if offset % 8 == 0:  # 8-byte aligned
                    ...
                    if (
                        0x08000000 <= potential_handler <= 0x08100000
                        or 0x20000000 <= potential_handler <= 0x20080000
                        or 0x00000000 <= potential_handler <= 0x00100000
                    ):
                        valid_entries = 1
                        ...
                        for next_cmd in range(2, 21):
                            if check_offset + 8 <= len(self.binary_data):
                                next_id = struct.unpack("<H", self.binary_data[check_offset : check_offset + 2])[0]
                                next_handler = struct.unpack(
                                    "<I", self.binary_data[check_offset + 4 : check_offset + 8]
                                )[0]
            ...
            for i, pos in enumerate(positions):
                ...
                if pos + 6 <= len(self.binary_data):
                    try:
                        ...
                        if (
                            0x08000000 <= handler_candidate <= 0x08100000
                            or 0x20000000 <= handler_candidate <= 0x20080000
                            or 0x00000000 <= handler_candidate <= 0x00100000
                        ):
                            print("  [OK] Handler address in valid ARM range")
                            ...
                            print("  [X] Handler address not in expected ARM range")

                    except (struct.error, IndexError):
                        print("  Parse error at offset")
