...
from config_and_logger import load_config, save_config, get_default_config
...
class TestSettingsPersistenceRootCause(unittest.TestCase):
    ...
    def _setup_parent_gui_variables(self):
        ...
        # Boolean variables
        bool_vars = [
            'autoconnect_var', 'auto_refresh_files_var', 'quit_without_prompt_var',
            'suppress_console_output_var', 'suppress_gui_log_output_var',
        ...
        # Numeric variables
        setattr(self.mock_parent, 'ai_temperature_var', 
                ctk.DoubleVar(value=self.initial_config.get('ai_temperature', 0.3)))
        setattr(self.mock_parent, 'ai_max_tokens_var', 
                ctk.IntVar(value=self.initial_config.get('ai_max_tokens', 4000)))
    ...
    def test_column_sorting_persistence_failure(self):
        ...
        self.assertEqual(reloaded_config['treeview_sort_col_id'], 'name',
                        "Column sort column should persist")
        self.assertEqual(reloaded_config['treeview_sort_descending'], False,
                        "Column sort direction should persist")
    ...
    def test_settings_not_saved_when_different_tab_selected(self):
        ...
        self.assertTrue(self.mock_parent.autoconnect_var.set.called,
                       "Autoconnect setting should be saved regardless of current tab")
        self.assertTrue(self.mock_parent.quit_without_prompt_var.set.called,
                       "Quit without prompt setting should be saved regardless of current tab")
        self.assertTrue(self.mock_parent.recording_check_interval_var.set.called,
    ...
    def test_apply_button_completely_broken(self):
        ...
        # Verify Apply worked
        self.assertTrue(self.mock_parent.autoconnect_var.set.called,
                       "Apply should save autoconnect setting")
        self.assertTrue(self.mock_parent.logger_processing_level_var.set.called,
                       "Apply should save logger level setting")
        self.assertTrue(self.mock_parent.appearance_mode_var.set.called,
                       "Apply should save appearance mode setting")
        
        # Verify config was saved
        self.assertEqual(self.mock_parent.config['autoconnect'], True,
                        "Apply should update config dictionary")
    ...
    def test_config_key_mapping_issues(self):
        ...
        for var_name, expected_config_key, test_value in test_cases:
            if var_name in dialog.local_vars:
                ...
                self.assertIn(expected_config_key, self.mock_parent.config,
    ...
    def test_save_config_function_called(self):
        ...
        with patch('settings_window.save_config') as mock_save_config:
            ...
            self.assertTrue(mock_save_config.called,
                           "save_config should be called when settings are applied")
            
            # Verify it was called with the updated config
            called_config = mock_save_config.call_args[0][0]
            self.assertEqual(called_config['autoconnect'], True,
                           "save_config should be called with updated config")
    ...
    def test_settings_persistence_across_app_restart(self):
        ...
        self.assertEqual(reloaded_config['autoconnect'], True,
                        "Autoconnect should persist across restart")
        self.assertEqual(reloaded_config['log_level'], 'DEBUG',
                        "Log level should persist across restart")
        self.assertEqual(reloaded_config['recording_check_interval_s'], 25,
                        "Recording interval should persist across restart")
