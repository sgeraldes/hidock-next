...
class TestSettingsPersistenceCriticalBugs(unittest.TestCase):
    ...
    def test_apply_button_not_working(self):
        ...
        self.assertTrue(
            self.mock_parent.logger_processing_level_var.set.called,
            "Apply should save logger level setting"
        )
        
        # Verify change tracking is reset after Apply
        self.assertFalse(
            dialog.settings_changed_tracker[0],
            "Change tracking should be reset after Apply"
        )
    ...
    def test_column_sorting_not_saved(self):
        ...
        # This test should FAIL initially (Red phase)
        self.assertEqual(
            mock_main_gui.config.get('treeview_sort_col_id'),
            'size',
            "Column sort column should be saved to config"
        )
        self.assertEqual(
    ...
    def test_settings_persistence_across_all_tabs(self):
        ...
        self.assertTrue(
            self.mock_parent.autoconnect_var.set.called,
            "General tab settings should be saved"
        )
        self.assertTrue(
            self.mock_parent.appearance_mode_var.set.called,
            "General tab settings should be saved"
        )
    ...
    def test_apply_vs_ok_behavior_difference(self):
        ...
        self.assertEqual(
            apply_calls,
            ok_calls,
            "Apply and OK should have identical save behavior"
        )
