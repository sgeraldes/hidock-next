...
def test_device_test_manager_isolation():
    ...
    test_1_isolated = (
        execution_order.index("test_1_start") < execution_order.index("test_1_end") < 
        execution_order.index("test_2_start") < execution_order.index("test_2_end")
    )
    test_2_isolated = (
        execution_order.index("test_2_start") < execution_order.index("test_2_end") < 
        execution_order.index("test_1_start") < execution_order.index("test_1_end")
...
async def test_concurrent_async_operations():
    ...
    for i in range(1, 4):
        ...
        assert all(not event.startswith(("start_", "end_")) or event == f"end_{i}" 
                  for event in between_events), f"Operation {i} was not isolated: {results}"
