...
class TestSettingsDialogEncryption:
    ...
    def test_generate_encryption_key_new(self, dialog_with_encryption_mocks):
        ...
        with patch('settings_window.ENCRYPTION_AVAILABLE', True), \
             patch('settings_window.Fernet') as mock_fernet, \
             patch('settings_window.os.path.exists', return_value=False), \
    ...
    def test_encrypt_api_key_success(self, dialog_with_encryption_mocks):
        ...
        with patch('settings_window.ENCRYPTION_AVAILABLE', True), \
             patch.object(dialog_with_encryption_mocks, '_generate_encryption_key', return_value=b'test-key'), \
             patch('settings_window.Fernet') as mock_fernet, \
             patch('settings_window.base64.b64encode', return_value=b'encrypted-data'):
    ...
    def test_decrypt_api_key_error(self, dialog_with_encryption_mocks):
        ...
        with patch('settings_window.ENCRYPTION_AVAILABLE', True), \
             patch.object(dialog_with_encryption_mocks, '_generate_encryption_key', return_value=b'test-key'), \
             patch('settings_window.Fernet') as mock_fernet, \
             patch('settings_window.base64.b64decode', side_effect=Exception("Decode error")):
...
class TestSettingsDialogApplyLogic:
    ...
    def test_perform_apply_settings_logic_success(self, dialog_with_apply_mocks):
        ...
        with patch('settings_window.save_config') as mock_save, \
             patch('settings_window.logger') as mock_logger:
    ...
    def test_perform_apply_settings_logic_directory_change(self, dialog_with_apply_mocks):
        ...
        with patch('settings_window.save_config'), \
             patch('settings_window.logger'), \
             patch('settings_window.Path') as mock_path:
...
class TestSettingsDialogDeviceIntegration:
    ...
    def test_on_device_selected_enhanced(self, dialog_with_device_mocks):
        ...
        dialog_with_device_mocks.local_vars = {
            "selected_vid_var": Mock(),
            "selected_pid_var": Mock()
        }
    ...
    def test_on_device_scan_complete(self, dialog_with_device_mocks):
        ...
        mock_devices = [
            Mock(is_hidock=True, status="available"),
            Mock(is_hidock=False, status="available")
        ]
