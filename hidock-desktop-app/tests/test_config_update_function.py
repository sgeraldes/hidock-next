...
class TestConfigUpdateFunction(unittest.TestCase):
    ...
    def test_update_config_settings_preserves_existing(self):
        ...
        with patch('config_and_logger._CONFIG_FILE_PATH', self.temp_config_path):
            ...
            self.assertEqual(final_config['autoconnect'], True,
                            "Original autoconnect setting should be preserved")
            self.assertEqual(final_config['treeview_sort_col_id'], 'name',
                            "Column sorting should be preserved")
            self.assertEqual(final_config['treeview_sort_descending'], False,
                            "Column sort direction should be preserved")
            
            # New settings should be applied
            self.assertEqual(final_config['log_level'], 'DEBUG',
                            "New log level should be applied")
            self.assertEqual(final_config['appearance_mode'], 'Dark',
                            "New appearance mode should be applied")
            
            # Default settings should still be there
            self.assertEqual(final_config['color_theme'], 'blue',
    ...
    def test_update_config_settings_handles_missing_file(self):
        ...
        with patch('config_and_logger._CONFIG_FILE_PATH', non_existent_path):
            ...
            self.assertEqual(config['autoconnect'], True,
                            "Updated settings should be saved")
            self.assertEqual(config['log_level'], 'DEBUG',
                            "Updated settings should be saved")
            
            # Default settings should also be there
            self.assertEqual(config['color_theme'], 'blue',
    ...
    def test_column_sorting_fix_with_update_function(self):
        ...
        with patch('config_and_logger._CONFIG_FILE_PATH', self.temp_config_path):
            ...
            self.assertEqual(config['treeview_sort_col_id'], 'size',
                            "Column sorting should persist through multiple updates")
            self.assertEqual(config['treeview_sort_descending'], True,
                            "Column sort direction should persist through multiple updates")
            
            # All other settings should also be there
            self.assertEqual(config['autoconnect'], True,
                            "Settings dialog changes should be saved")
            self.assertEqual(config['log_level'], 'DEBUG',
                            "Settings dialog changes should be saved")
            self.assertEqual(config['appearance_mode'], 'Dark',
                            "Appearance changes should be saved")
            self.assertEqual(config['color_theme'], 'green',
                            "Theme changes should be saved")
    ...
    def test_save_config_still_works_for_full_configs(self):
        ...
        with patch('config_and_logger._CONFIG_FILE_PATH', self.temp_config_path):
            ...
            self.assertEqual(saved_config['autoconnect'], True,
                            "Full config save should work")
            self.assertEqual(saved_config['log_level'], 'DEBUG',
                            "Full config save should work")
            self.assertEqual(saved_config['treeview_sort_col_id'], 'name',
                            "Full config save should work")
    ...
    def test_mixed_save_and_update_operations(self):
        ...
        with patch('config_and_logger._CONFIG_FILE_PATH', self.temp_config_path):
            ...
            self.assertEqual(config['autoconnect'], True,
                            "Autoconnect should be preserved")
            self.assertEqual(config['treeview_sort_col_id'], 'name',
                            "Column sorting should be preserved")
            self.assertEqual(config['treeview_sort_descending'], False,
                            "Column sort direction should be preserved")
            self.assertEqual(config['log_level'], 'DEBUG',
                            "Log level should be preserved")
            self.assertEqual(config['appearance_mode'], 'Dark',
                            "Appearance mode should be preserved")
