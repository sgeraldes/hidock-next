...
import { ChevronDown, ChevronRight, Users, Check, HelpCircle, Mic, CalendarCheck } from 'lucide-react'
...
/** Icon for the signal behind a best guess. */
function SignalIcon({ method }: { method: BucketRecording['method'] }) {
  ...
  if (method === 'attendee-context') return <CalendarCheck className="h-3 w-3 flex-shrink-0" aria-hidden />
  return <HelpCircle className="h-3 w-3 flex-shrink-0" aria-hidden />
}

/** A single recording row with a per-recording override select. */
