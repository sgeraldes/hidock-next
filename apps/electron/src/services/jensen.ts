...
// ============================================================
// Singleton — IPC-backed (renderer)
// ============================================================
//
// getJensenDevice() previously returned a WebUSB-backed JensenDevice.
// It now returns a JensenIpcClient that delegates to the main-process
// singleton via window.electronAPI.jensen.*.
//
// The return type is kept as JensenDevice so hidock-device.ts needs no
// changes: JensenIpcClient exposes the same public surface.
//
// The old WebUSB singleton is still available for tests that need the
// real JensenDevice class (e.g. jensen.test.ts tests the protocol directly).
// ============================================================

import { JensenIpcClient, getJensenIpcClient } from './jensen-ipc-client'
export { JensenIpcClient }

export function getJensenDevice(): JensenDevice {
  return getJensenIpcClient() as unknown as JensenDevice
}

/** @deprecated Use getJensenDevice() which is now IPC-backed */
