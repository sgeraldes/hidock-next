...
class OAuth2TokenManager:
    ...
    def _ensure_encryption_key(self):
        ...
        try:
            ...
            logger.error("OAuth2TokenManager", "_ensure_encryption_key",
    ...
    def save_tokens(self, provider: str, tokens: Dict) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.info("OAuth2TokenManager", "save_tokens",
    ...
    def _load_tokens_file(self) -> Dict:
        ...
        try:
            ...
            logger.warning("OAuth2TokenManager", "_load_tokens_file",
                          f"Error reading tokens file: {e}, returning empty dict")
            return {}
    ...
    def is_token_valid(self, provider: str) -> bool:
        ...
        try:
            ...
            if is_valid:
                ...
                logger.debug("OAuth2TokenManager", "is_token_valid",
                            f"{provider} token expired at {tokens['expires_at']}")
    ...
    def get_access_token(self, provider: str) -> Optional[str]:
        ...
        logger.info("OAuth2TokenManager", "get_access_token",
                   f"{provider} token expired, attempting refresh")

        if self.refresh_token(provider):
            tokens = self.load_tokens(provider)
            return tokens.get('access_token') if tokens else None

        logger.warning("OAuth2TokenManager", "get_access_token",
                      f"Failed to refresh {provider} token, re-authentication needed")
        return None
    ...
    def refresh_token(self, provider: str) -> bool:
        ...
        try:
            ...
            if not tokens or 'refresh_token' not in tokens:
                logger.warning("OAuth2TokenManager", "refresh_token",
                              f"No refresh token found for {provider}")
                return False
            ...
            logger.info("OAuth2TokenManager", "refresh_token",
                       f"Successfully refreshed {provider} token")
            return True
