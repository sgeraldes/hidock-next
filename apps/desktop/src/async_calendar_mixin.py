...
class AsyncCalendarMixin:
    ...
    def _enhance_single_file_with_calendar_data(self, file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            meeting = self._calendar_integration.find_meeting_for_recording(file_datetime, tolerance_minutes=tolerance_minutes)
    ...
    def _enhance_files_with_batch_calendar_data(self, files_dict: List[Dict]) -> List[Dict]:
        ...
        for chunk_start_date, chunk_files in date_chunks.items():
            try:
                ...
                logger.info("AsyncCalendar", "batch_enhance", f"Attempting individual processing for {len(chunk_files)} files from failed chunk")
                for file_data in chunk_files:
                    try:
                        # Try to process each file individually
                        enhanced_file = self._enhance_single_file_with_calendar_data(file_data)
                        enhanced_files.append(enhanced_file)
                        logger.debug("AsyncCalendar", "batch_enhance", f"Individual processing succeeded for {file_data.get('name', 'unknown')}")
                    except Exception as individual_error:
                        logger.debug("AsyncCalendar", "batch_enhance", f"Individual processing failed for {file_data.get('name', 'unknown')}: {individual_error}")
