...
- 2026-07-15 ~05:xx — **CREDIT INCIDENT (orchestrator error).** Subagents were spawned Opus but driven via SendMessage resumes while the session model was Fable 5; resumes inherit the SESSION model, so all fix-round work ran on Fable 5 and exhausted the user's Fable 5 credits. Both lane agents died mid-final-round. **Preserved as WIP commits** (unverified, DO-NOT-integrate-labeled): lane/a round-4 `<pending sha>` (reset→initiation, orchestrator gate-transition, process-lifetime IPC wrapper), lane/b round-5 `3764970b` (atomic backend group-unmerge, full rollback). Both agents reported tests passing before death. Session switched to Opus. Going forward: spawn fresh Opus agents per round OR keep session on Opus so resumes inherit it. See memory feedback-subagent-model-pinning.
