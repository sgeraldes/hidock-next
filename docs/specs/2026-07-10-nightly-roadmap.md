...
- [ ] **H2 (P0)** Waveform must **MATCH THE MOCKUP**: colored bars + a sentiment curve on a subtle gradient panel + **numbered circular markers sitting ON the curve** + the event list below. It currently looks nothing like it.
- [ ] **H3 (P0)** Action items are **DUPLICATED** — shown in the timeline event-list AND again in the transcript/summary. De-dup: remove them from the transcript (keep them in the summary / event-list).
...
- [ ] **H5 (P0)** Kill the ugly **"Loading waveform…" overlay**. Generate peaks ONCE, **CACHE ON DISK**, load instantly from cache each open; only a brief animated placeholder text while (rarely) computing — never a half-drawn wave with overlaid text.
- [ ] **H6 (P0)** **"Transcript not available" + no-color waveform** when selecting a transcribed recording via the sidebar Library nav — the transcript + speakers ARE present. Fix load-on-navigation so both render.
