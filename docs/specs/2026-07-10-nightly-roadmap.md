...
- [ ] **F2 (P1)** **Assistant placement matrix dogfood + overlay layout bug.** Owner's screenshot shows the FLOATING overlay rendering the Chat squeezed into a broken two-column layout (page content bleeding behind, send column crushed to ~80px with wrapped caption text). Walk ALL four combos — floating/embedded × left/right — fix the overlay layout, verify pin-to-embed and collapse behaviors, both themes, live via CDP.
